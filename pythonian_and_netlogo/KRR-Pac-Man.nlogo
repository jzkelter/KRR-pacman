; change nearest pellet and nearest ghost to only report the direction to move in to get there.
...
to setup-python
  py:setup py:python
  py:run "import os"
  output-print (py:runresult "os.getcwd()")
  py:run "from pac_agent import PacAgent"
  py:run "a = PacAgent(host='localhost', port=9000, localPort=8950)"
end
...
;; ***********Move and get/give knowledge to companion procedures*************
to move-pacman  ;; Observer Procedure
  ask pacmans
  [ ;; move forward unless blocked by wall
    send-companion-world-state
    let old-heading heading
    set-new-heading ask-companion-direction-to-face ; This is gotten through companions
...
to set-new-heading [dir-to-face]
  print dir-to-face
  (ifelse
    dir-to-face = "Up" [ set new-heading 0]
...
to-report direction
  report (ifelse-value
    [heading] of pacman 0 = 0 ["Up)"]
    [heading] of pacman 0 = 90 ["Right)"]
    [heading] of pacman 0 = 180 ["Down)"]
    [heading] of pacman 0 = 270 ["Left)"]
   )
end
...
to send-companion-world-state  ;; pacman function
  ...
  ;; Wall facts
  if wall-at-heading 0 [set facts lput "(wall Up)" facts]
  if wall-at-heading 90 [set facts lput "(wall Right)" facts]
  if wall-at-heading 180 [set facts lput "(wall Down)" facts]
  if wall-at-heading 270 [set facts lput "(wall Left)" facts]

  ;; Ghost facts
  if ghost-at-heading 0 [set facts lput "(ghost Up)" facts]
  if ghost-at-heading 90 [set facts lput "(ghost Right)" facts]
  if ghost-at-heading 180 [set facts lput "(ghost Down)" facts]
  if ghost-at-heading 270 [set facts lput "(ghost Left)" facts]
...
to-report wall-at-heading [head]
  report ([pcolor] of patch-at-heading-and-distance head 1) = blue
end
...
to-report find-nearest-ghost-dirs
  report find-nearest-breed-directions ghosts
end

to-report find-nearest-breed-directions [the-breed]
  let max-ghost-depth 10
  let nearest-breed-dirs (list)
  let depth 1
  while [empty? nearest-breed-dirs and (depth < max-ghost-depth or the-breed != ghosts)] [
    (foreach [0 90 180 270] ["Up)" "Right)" "Down)" "Left)"]
      [[head dir-string] ->
        if not wall-at-heading head [
          let patches-in-dir nearest-patches-in-dir head depth
          if any? patches-in-dir with [any? turtles-here with [breed = the-breed]] [set nearest-breed-dirs lput dir-string nearest-breed-dirs]
        ]
      ]
    )
   set depth depth + 1
  ; if (depth > max-ghost-depth and the-breed = ghosts) [stop]
  ]
  report nearest-breed-dirs
end



to-report find-nearest-pellets
  report find-nearest-breeds-helper (patch-set) 1 pellets
end

to-report find-nearest-breeds-helper [nearest-breed-patches depth the-breed]
  ifelse any? nearest-breed-patches [
    report nearest-breed-patches
  ] [
    set nearest-breed-patches (nearest-patches patch-here depth) with [any? turtles-here with [breed = the-breed]]
    report find-nearest-breeds-helper nearest-breed-patches (depth + 1) the-breed
  ]
end

to-report ghost-at-heading [head]
  let patches-ahead nearest-patches-in-dir head 4
  let retval false
  ask patches-ahead [
    if any? ghosts-here [set retval true]
  ]
  report retval
end

to-report nearest-patches-in-dir [dir depth]
  let start-patch patch-at-heading-and-distance dir 1
  report nearest-patches start-patch depth
end

to-report nearest-patches [start-patch depth]
  ;; reports all the patches within depth steps from the start-patch without going through walls or the patch pacman is currently on
  let near-patches (patch-set start-patch) ; patch-at-heading-and-distance head 2 patch-at-heading-and-distance head 3)
  let my-pxcor  pxcor
  let my-pycor pycor
  repeat (depth - 1) [
    ask near-patches [
      if pcolor != blue[
        set near-patches (patch-set near-patches (neighbors4 with [pcolor != blue and not (pxcor = my-pxcor and pycor = my-pycor)]))
      ]
    ]
  ]
  report near-patches
...
283
send-companion-world-state
ask pacman 0 [send-companion-world-state]
NIL
1
T
OBSERVER
...
219
341
ask-companion-direction-to-face
print ask-companion-direction-to-face
NIL
1
T
OBSERVER
